...
![整体效果](docs/images/screenshot.png)

> 深色主题完整界面：控制面板（左）+ Canvas 地图（中）+ 串口/协议面板（右）+ 浮动日志/视频面板（下）

### 功能标注总览

![功能标注总览](docs/images/feature-overview.png)

> 三大区域标注：控制面板（路径规划 / 障碍物 / 比赛步骤）、地图区域（48×48 格栅 / 彩色分段路径 / 实时小车）、右面板（串口连接 / 协议监控 / 实时位姿）

---

## 🗺️ 地图功能详解

![地图区域功能标注](docs/images/feature-map.png)

> 地图区域标注说明：48×48 格栅、启停区 / 加工台 / 暂存区 / 原料区 / 二维码区位置、8 段彩色分段路径、实时小车位姿渲染（麦轮 + 车头方向）
...
![控制面板功能标注](docs/images/feature-control.png)

> 控制面板功能详解：启停区选择、A* 路径规划（含安全膨胀）、障碍物设置、比赛步骤控制、统计数据、主题切换
